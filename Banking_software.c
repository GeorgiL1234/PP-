...
//хешира даден низ, като увеличава ASCII стойността на всеки символ с определен ключ
...
//комбинира две низови стойности сол и парола по такъв начин, че ги смесва буква по буква с крайна цел да се създаде нов низ, който съдържа символи и от солта, и от паролата, като се редуват.
...
//инициализира нова хеш-таблица със зададен брой кофи и максимален брой елементи в кофа.
...
//проверява дали даден свързан списък съдържа определен потребител по зададено потребителско име
...
//създава нов възел за свързан списък
...
//добавя нов възел към края на свързания списък.
...
//прехвърля всички елементи от една хеш-таблица в друга
...
//създава нова хеш-таблица с двойно по-голям брой кофи и прехвърля всички елементи от старата хеш-таблица в новата.
...
// добавя нов елемент в хеш-таблицата и, ако е необходимо, увеличава размера на хеш-таблицата, за да поддържа ефективността на операциите.
...
//генерира случайна символен низ с дължина
...
//генерира защитни данни за нова парола
...
//служи за записване на данни за потребител в текстов файл.
...
//аделя динамична памет за символен низ
...
//използва за въвеждане на потребителско име (username) от имейл адрес.
...
// служи за въвеждане на парола от потребителя, като символите на паролата се показват като звездички ('*') в конзолата.
...
//проверка дали съществува потребител с даденото потребителско име
...
//служи за регистрация на нов потребител в системата.
...
//използва сее за намиране на потребител в хеш таблицата по зададено потребителско име
...
//има за цел да автентикира потребител в системата чрез въвеждане на потребителско име и парола
...
// има за цел да запише цялата съдържаща се информация в хеш таблицата във файл
...
//има за цел да позволи на потребител да внесе определена сума пари на своя акаунт
...
//позволява на потребителя да изтегли определена сума пари от своя акаунт
...
//се използва за добавяне на нова транзакция към края на опашката от транзакции
...
void transaction(struct TransactionQueue* q, struct Account* account1, struct Account* account2){
    ...
    do{
        ...
        printf(ANSI_COLOR_PINK);
        printf(ANSI_COLOR_BLACK, "Enter the amount of money you want to transfer: ");
...
//има за цел да позволи на потребителя да извърши транзакция между два акаунта
...
void popQueue(struct TransactionQueue* q) {
    if(q->head == NULL)
        return;
    if(q->head->fromAccount->money < q->head->amountToTransfer)
        printf("You do not have enough money to process this transaction!\n");
    else{
...
//има за цел да обработи и изпълни първата транзакция от опашката от транзакции
    ...
    while (1) {
        ...
        switch (option) {
            ...
            case 3: system("cls");
                    ...
                    char user[MAX_USERNAME_LENGTH];
                    printf(ANSI_COLOR_PINK "Enter the username of the person you want to transfer money to: ");
                    scanf("%s", user);
...
//управляват интеракцията с потребителя
...
// зареждане на данни от файл и добавянето им в хеш таблицата
...
//инициализира и връща указател към нова опашка от транзакции

void load_transactions_from_file(struct TransactionQueue* q, const char* path, struct Hashtable* table) {
    FILE *f = fopen(path, "r");
    if (f == NULL) {
    ...
    while (fgets(line, MAX_LINE_LENGTH, f) != NULL) {
        ...
        if (node1 != NULL && node2 != NULL) {
            ...
            fprintf(stderr, "Error: One of the users not found in the table.\n");
        }
...
//има за цел да зареди транзакции от текстов файл в структурата
...
// има за цел да генерира хеш стойност на база на съдържанието на текстов файл
...
// сравнява два текстови файла по техните хеш стойности
